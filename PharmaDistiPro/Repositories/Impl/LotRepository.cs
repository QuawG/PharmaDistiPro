...
namespace PharmaDistiPro.Repositories.Impl
{
    public class LotRepository : RepositoryBase<Lot>, ILotRepository 
    {
        ...
        {
            return await _context.Lots.Include(l => l.ProductLots).FirstOrDefaultAsync(l => l.LotCode == LotCode);
        }
        public async Task<List<Lot>> GetLotList()
        {
            return await _context.Lots.Include(l => l.ProductLots).ToListAsync();
        }
